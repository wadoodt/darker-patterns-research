...
// File paths
const combinedPath = path.join(__dirname, '../src/locales/support.combined.json');
const enPath = path.join(__dirname, '../src/locales/en/pages/support.json');
const esPath = path.join(__dirname, '../src/locales/es/pages/support.json');
const dtsPath = path.join(__dirname, '../src/locales/en/pages/support.d.ts');
...
function verifyTranslations() {
    ...
    if (typeof keys === 'object' && !Array.isArray(keys)) {
      for (const [key, value] of Object.entries(keys)) {
        if (typeof value === 'object' && !Array.isArray(value)) {
          // Skip direct comparison of nested objects
          continue;
        }
  ...
  // Fix Spanish contact form title mismatch
  if (combined.translations['contact_form.title'].es !== es.contact_form.title) {
    console.error(`Mismatch for contact_form.title (es):
  Combined: "${combined.translations['contact_form.title'].es}"
  Source:   "${es.contact_form.title}"`);
    errors++;
  }
...
// Run verification
console.log('Verifying support translations...');
const errorCount = verifyTranslations();

if (errorCount === 0) {
  console.log('✅ All support translations match between combined and source files');
} else {
